...
const RightSideBar = ({user,transactions,banks}:RightSidebarProps) => {
  return (
    ...
    //              <span className="text-[25px] font-bold text-blue-500">{user?.firstName[0]}</span>
    //             </div>
    ...
    //     </section>
    // </aside>
    <aside className='min-w-[330px] hideRightBar bg-white shadow-md h-full'>
  <section className='flex flex-col pb-8'>
    <div className='h-[150px] w-full bg-gradient-mesh bg-cover bg-no-repeat'></div>
    <div className='relative flex px-6 justify-center'>
      <div className='flex items-center justify-center absolute left-3 -top-5 w-[60px] h-[60px] rounded-full bg-gray-100 border-8 border-white p-2 shadow-sm'>
        <span className="text-[25px] font-bold text-blue-500">{user?.firstName[0]}</span>
      </div>
      <div className='flex flex-col pt-24 items-center md:items-start'>
        <h1 className='text-24 font-semibold text-gray-900'>
          {`${user.firstName} ${user.lastName}`}
        </h1>
        <p className='text-16 font-normal text-gray-600'>
  ...
  <section className='flex flex-col justify-between gap-8 px-6 py-8'>
    <div className='flex w-full justify-between items-center'>
      <h2 className='header-2 text-xl font-semibold text-gray-900'>
        My banks
      </h2>
      <Link href="/connect-Bank" className="flex items-center gap-2 hover:text-blue-500 transition">
        <Image src='/icons/plus.svg' width={20} height={20} alt="plus"></Image>
        <h2 className='text-14 font-semibold text-gray-600'>
          Add Bank
        </h2>
